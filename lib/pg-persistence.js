...
module.exports = class PgPersistence {
  ...
  async loadTodoList(todoListId) {
    ...
    let resultTodoList = dbQuery(FIND_TODOLIST, this.username, todoListId);
  ...
  async completeAllTodos(todoListId) {
    const COMPLETE_ALL = `UPDATE todos SET done = true
                          WHERE username = $1 AND todolist_id = $2`;

    let result = await dbQuery(COMPLETE_ALL, this.username, todoListId);
    return result.rowCount > 0;
  }
